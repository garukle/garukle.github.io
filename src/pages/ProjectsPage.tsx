...
export default function ProjectsPage({ onBack, entering, exiting }: Props) {
  ...
  return (
    <PageSkeleton
      ...
      exiting={exiting}
    >
      <div className="absolute inset-0 projects-bg">
        {/* Filter dropdown */}
        <div
          className="absolute"
          style={{
            ...
            border: `solid #f8e8a0`,
            borderTopColor: '#f8e8a0',
            borderRightColor: '#b86870',
            borderBottomColor: '#b86870',
            borderLeft: 0
          }}
        >
          <button
          ...
          {filterOpen && !selected && (
            ...
            >
              {(["ALL", "CS", "ART"] as Filter[]).map((f) => (
                <button
                  key={f}
                  className="w-full text-left px-1 hover:bg-[#b86870]"
                  style={{
                    fontSize: s(9),
                    color: filter === f ? "#f8e8a0" : "#f8f8f8",
                    padding: `${s(2)} ${s(4)}`,
                    textShadow: `${s(1)} 0 0 rgba(0,0,0,0.5)`
                  }}
                  onClick={() => {
          ...
          {preview ? (
            ...
            >
              <div style={{ paddingTop: s(6) }}>
                <div
                  style={{ fontSize: s(9), color: "#f8f8f8", textShadow: `${s(1)} 0 0 rgba(0,0,0,0.5)`, textAlign: "left" }}
                >
                  No. registered
                </div>
                <div
        ...
        <div
          ...
          style={{
            ...
            background: "#f0c078",
            boxShadow: `${s(-1)} 0 0 #304048, 0 ${s(1)} 0 #304048, 0 ${s(-1)} 0 #304048`,
            border: `${s(3)} solid #f8e8a0`,
            borderTopColor: '#f8e8a0',
            borderLeftColor: '#f8e8a0',
            borderBottomColor: '#b86870',
            borderRight: 0
          }}
        >
          ...
          {scrollTop && (
            ...
            >
              <span
                className="material-symbols-outlined"
                style={{ fontSize: s(15), color: "#589880", textShadow: `${s(1)} 0 0 rgba(0,0,0,0.5)` }}
              >
                arrow_drop_up
              </span>
            </div>
          ...
          >
            ...
            {isHovered && (
              ...
              >
                <span
                  className="material-symbols-outlined"
                  style={{ fontSize: s(15), color: "#589880", lineHeight: 1 }}
                >
                  arrow_right
                </span>
              </div>
            ...
            <span
              style={{ marginLeft: s(3), color: "#f8f8f8", textShadow: `${s(1)} 0 0 rgba(0,0,0,0.5)`, fontSize: s(12) }}
            >
              {p.category}
            </span>
            <span style={{ marginLeft: s(3), color: "#f8f8f8", textShadow: `${s(1)} 0 0 rgba(0,0,0,0.5)`, fontSize: s(12) }}>{p.name}</span>
            <span
              className={`material-symbols-outlined ${st.color} shrink-0`}
              style={{ fontSize: s(6), marginLeft: "auto" }}
            >
              {st.icon}
            </span>
          </button>
...
function DetailView({
  ...
  return (
    <div className="flex flex-col" style={{ gap: s(4) }}>
      ...
      {/* Sections */}
      <div className="flex flex-col" style={{ gap: s(2), paddingLeft: s(6) }}>
        {project.sections.map((section) => (
          <div key={section.label}>
            <div
              style={{ fontSize: s(12), color: "#f84810", textShadow: `${s(1)} 0 0 rgba(0,0,0,0.5)`, marginBottom: s(1) }}
            >
              {section.label}
            </div>
            <div style={{ fontSize: s(10), color: "#f8f8f8", textShadow: `${s(1)} 0 0 rgba(0,0,0,0.5)` }}>
              {section.content}
            </div>
          </div>
        ))}
